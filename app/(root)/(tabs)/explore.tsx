...
const Explore = () => {
  ...
  const applyGlobalFilters = (props: any[]) => {
    ...
    console.log(`Applied global filters: ${props.length} -> ${filtered.length} properties`);
    return filtered;
  };
        ...
        ListHeaderComponent={() => (
          <View className="px-5">
            ...
            <View className="mt-5">
              ...
              {filters.types.length > 0 || filters.bedrooms > 0 || filters.bathrooms > 0 || filters.priceRange[0] > 100 || filters.priceRange[1] < 500 || filters.sizeRange[0] > 1000 || filters.sizeRange[1] < 3000 ? (
                <View className="bg-primary-100 rounded-lg p-3 mt-3">
                  <Text className="text-sm font-rubik-medium text-primary-200 mb-2">Active Filters:</Text>
                  <View className="flex-row flex-wrap gap-2">
                    {filters.types.length > 0 && (
                      <View className="bg-white px-2 py-1 rounded-full">
                        <Text className="text-xs text-black-300">Types: {filters.types.join(', ')}</Text>
                      </View>
                    )}
                    {filters.bedrooms > 0 && (
                      <View className="bg-white px-2 py-1 rounded-full">
                        <Text className="text-xs text-black-300">Bedrooms: {filters.bedrooms}+</Text>
                      </View>
                    )}
                    {filters.bathrooms > 0 && (
                      <View className="bg-white px-2 py-1 rounded-full">
                        <Text className="text-xs text-black-300">Bathrooms: {filters.bathrooms}+</Text>
                      </View>
                    )}
                    {(filters.priceRange[0] > 100 || filters.priceRange[1] < 500) && (
                      <View className="bg-white px-2 py-1 rounded-full">
                        <Text className="text-xs text-black-300">Price: ${filters.priceRange[0]} - ${filters.priceRange[1]}</Text>
                      </View>
                    )}
                    {(filters.sizeRange[0] > 1000 || filters.sizeRange[1] < 3000) && (
                      <View className="bg-white px-2 py-1 rounded-full">
                        <Text className="text-xs text-black-300">Size: {filters.sizeRange[0]} - {filters.sizeRange[1]} sq ft</Text>
                      </View>
                    )}
                  </View>
                </View>
