...
// Demo data with clear default values
const demoFilters = {
  priceRange: [200, 600] as [number, number],
  sizeRange: [1500, 3000] as [number, number],
  types: ['House', 'Apartment'] as string[],
  bedrooms: 2,
  bathrooms: 1,
};
...
// Subcomponents (same as original but simplified)
...
const RangeSlider = ({
  ...
  const handleTouchMove = (event: any) => {
    if (activeThumb === null || containerWidth === 0) return;
...
interface FilterModalDemoProps {
  visible: boolean;
  onClose: () => void;
}

export const FilterModalDemo = ({ visible, onClose }: FilterModalDemoProps) => {
  const [filters, setFilters] = useState({
    priceRange: demoFilters.priceRange,
    sizeRange: demoFilters.sizeRange,
    types: demoFilters.types,
    bedrooms: demoFilters.bedrooms,
    bathrooms: demoFilters.bathrooms,
  });

  console.log('FilterModalDemo: Using demo data', filters);

  const handleReset = () => {
    console.log('FilterModalDemo: Reset to demo defaults');
    setFilters({
      priceRange: demoFilters.priceRange,
      sizeRange: demoFilters.sizeRange,
      types: demoFilters.types,
  ...
  const handleApply = () => {
    console.log('FilterModalDemo: Applied filters', filters);
    onClose();
  };
        ...
        >
          <SafeAreaView className="flex-1">
            ...
            <View className="flex-row items-center justify-between px-6 mb-6">
              ...
              <Text className="text-black-300 font-rubik-extra-bold text-xl">Filter (Demo)</Text>
            ...
            <View className="px-6 pb-4">
              ...
              >
                <Text className="text-white font-rubik-extra-bold text-base">Set Filter (Demo)</Text>
              </TouchableOpacity>
            </View>
          </SafeAreaView>
        </Animated.View>
